...
class TouchScroller(BasicScroller):
    ...
    middle_passthrough = BooleanProperty(True)
    ...
    def do_touch_down(self, touch):
        if self.collide_point(*touch.pos):
            ...
            if touch.button and 'button' in touch.profile and touch.button.startswith('scroll'):
                if self.allow_wheel:
                    touch.grab(self)
                    self._touch = touch
                    btn = touch.button
            ...
            if self.allow_middle_mouse and 'button' in touch.profile and touch.button == 'middle':
                return not self.middle_passthrough
    ...
    def do_touch_move(self, touch):
        ...
        if middle_button and self.middle_passthrough:
            return super().do_touch_move(touch)
        else:
            return True
